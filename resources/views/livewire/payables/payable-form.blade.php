        ...
        <div class="relative inline-block align-bottom bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-6 border-b border-slate-100 dark:border-slate-700/50">
                <h3 class="text-xl font-semibold text-slate-800 dark:text-white" id="modal-title">
                    {{ $obligationId ? 'Editar Conta a Pagar' : 'Nova Conta a Pagar' }}
                </h3>
                @if($isInstallment)
                ...
                <div class="px-6 py-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" id="title" wire:model="title" 
                               class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm {{ ($isInstallment || $isPaidOrCancelled) ? 'opacity-60 cursor-not-allowed' : '' }}" 
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <!-- Category -->
                        <div>
                            <label for="category_id" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Categoria (Despesa)</label>
                    ...
                    <div>
                        ...
                        <textarea id="notes" wire:model="notes" rows="2" 
                                  class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
