        ...
        <div class="relative inline-block align-bottom bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-6 border-b border-slate-100 dark:border-slate-700/50">
                <h3 class="text-xl font-semibold text-slate-800 dark:text-white" id="modal-title">
                    {{ $budgetId ? __('Editar Orçamento') : __('Novo Orçamento') }}
                </h3>
            </div>
            ...
            <form wire:submit.prevent="save">
                <div class="px-6 py-6 space-y-5">
                    ...
                    <div>
                        <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nome do Orçamento</label>
                        ...
                        @error('name') <span class="mt-1 text-sm text-rose-500">{{ $message }}</span> @enderror
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="period_start" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Data de Início</label>
                            <input type="date" id="period_start" wire:model="period_start" 
                                   class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @error('period_start') <span class="mt-1 text-sm text-rose-500">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label for="period_end" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Data de Fim</label>
                            <input type="date" id="period_end" wire:model="period_end" 
                                   class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @error('period_end') <span class="mt-1 text-sm text-rose-500">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div>
                        <label for="target_amount" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Teto Global (Opcional)</label>
                        <input type="number" step="0.01" id="target_amount" wire:model="target_amount" 
                               class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                               placeholder="0.00">
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            Deixe 0 para utilizar a soma das categorias como limite global.
                        </p>
                        @error('target_amount') <span class="mt-1 text-sm text-rose-500">{{ $message }}</span> @enderror
                    </div>
                </div>
