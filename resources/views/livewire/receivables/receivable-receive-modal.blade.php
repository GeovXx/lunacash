        ...
        <div class="relative inline-block align-bottom bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-slate-200 dark:border-slate-700">
            ...
            <form wire:submit.prevent="receive">
                <div class="px-6 py-6 space-y-5">
                    ...
                    <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800/30 mb-4">
                        <p class="text-sm text-blue-800 dark:text-blue-400 font-medium text-center">Valor a Receber</p>
                        <p class="text-2xl font-bold text-blue-600 dark:text-blue-500 text-center mt-1">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" id="pay_amount" wire:model="amount" 
                               class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('amount') <span class="mt-1 text-sm text-rose-500">{{ $message }}</span> @enderror
                        <p class="mt-1 text-xs text-slate-500">O recebimento parcial não é suportado no momento. O valor deve ser exato.</p>
                    </div>

                    <!-- Date -->
                    <div>
                        <label for="transaction_date" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Data do Recebimento</label>
                    ...
                    <!-- Account -->
                    <div>
                        <label for="pay_account_id" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Conta de Destino (Recebedora)</label>
                        <select id="pay_account_id" wire:model="account_id" 
                                class="w-full rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @foreach($accounts as $account)
                                <option value="{{ $account->id }}">{{ $account->name }} (Saldo atual: R$ {{ number_format($account->balance, 2, ',', '.') }})</option>
                            @endforeach
                        </select>
                ...
                <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
